...
public class CadastroForm extends GridPane {
	...
	public CadastroForm build() {
		...
		nomeField.setPromptText("Nome & Sobrenome");
		nomeField.setPrefWidth(300);
		nomeField.getStyleClass().add("camposCadastro-text-field");
		...
		emailField.getStyleClass().add("camposCadastro-text-field");

		senhaField = new PasswordField();
		senhaField.setPromptText("Senha");
		senhaField.setPrefWidth(300);
		senhaField.getStyleClass().add("camposCadastro-text-field");
		...
		aplicarCapitalizacao(nomeField);
	...
	private HBox criarCampoComIcone(Image icone, TextField campo) {
		...
		imagem.setFitWidth(26);
		imagem.setFitHeight(26);
	...
	private void aplicarCapitalizacao(TextField field) {
		field.textProperty().addListener((obs, oldValue, newValue) -> {
			if (newValue == null || newValue.isEmpty())
				return;

			// Divide por espaços preservando múltiplos espaços
			String[] partes = newValue.split("(?<= )"); // mantém os espaços no array
			StringBuilder resultado = new StringBuilder();

			for (String parte : partes) {
				if (parte.trim().isEmpty()) {
					resultado.append(parte); // apenas espaço
				} else {
					resultado.append(Character.toUpperCase(parte.charAt(0)));
					if (parte.length() > 1) {
						resultado.append(parte.substring(1).toLowerCase());
					}
				}
			}

			// Evita loop infinito (só altera se for diferente)
			String capitalizado = resultado.toString();
			if (!capitalizado.equals(newValue)) {
				field.setText(capitalizado);
				field.positionCaret(capitalizado.length()); // mantém o cursor no fim
	        }
		});
	}
