...
public class GraficoAtivos {
	...
	// ==========================================================
	// Exibe um gráfico de pizza (donut) com a distribuição da carteira por categoria
	// e uma lista lateral com percentual + valor em R$ por categoria.
	// Mantém as mesmas fatias (PieChart.Data) para evitar flicker e reordenação.
	...
	private static final Map<String, String> COR_POR_CATEGORIA = Map.of("Ações", "#ff7981", "FIIs", "#88FFC3", "Renda Fixa", "#50A0FF", "Criptomoedas", "#fff369", "ETFs", "#02757B", "Tesouro Direto",
			"#999999"
	);
	...
	public GraficoAtivos() {
		...
		// Aplica estilos após render
		Platform.runLater(() -> {
			aplicarEstiloLabelsGrafico(Color.WHITE);
	...
	public void atualizar(Map<String, Double> totalPorCategoria) {
		Platform.runLater(() -> {
			...
			// 2) Lista lateral com percentuais reais
			...
			for (PieChart.Data d : ordenado) {
				...
				double pct = (total > 0) ? (v / total) * 100.0 : 0.0;
			...
			aplicarCoresDepoisDoRender();
			aplicarEstiloLabelsGrafico(Color.web("#424242"));
		});
	}
	...
	private void aplicarEstiloLabelsGrafico(Color cor) {
